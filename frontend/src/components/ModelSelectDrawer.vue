<template> <v-sheet class="mx-auto" elevation="8" style="height: calc(100vh - 165px); overflow-y: scroll">
    ...
    <v-expand-transition>
      <v-expansion-panels variant="inset" class="my-4" expand v-model="panel">
        <v-expansion-panel
          v-for="childModel in selectedParent.children"
          :key="childModel.shortName"
        >
          ...
          <v-expansion-panel-text>
            ...
            <v-btn
              color="primary"
              @click="selectModel(childModel)"
              :disabled="isDisabled(childModel)"
              >Continue with {{ childModel.shortName }}</v-btn
            >
          </v-expansion-panel-text>
        </v-expansion-panel>
      </v-expansion-panels>
