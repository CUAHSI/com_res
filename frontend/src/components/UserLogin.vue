  ...
  <v-dialog v-if="!auth.isLoggedIn" width="500">
    ...
    <template v-slot:default="{ isActive }">
      <v-card class="cd-login">
        ...
        <v-card-text>
          <p class="text-body-1">
            User accounts in the com_res are managed using CUAHSI SSO. You will be redirected to
            CUAHSI SSO where you can login or create an account.
          </p>
        </v-card-text>
        <v-divider></v-divider>
        <v-card-actions>
  ...
  <div v-else>
    ...
    <template v-else>
      <v-card class="nav-items ma-2" :elevation="1">
        ...
        <v-btn @click="logOutUser" :prepend-icon="mdiAccountKey" :elevation="1" class="ma-2"
